...
//
// Uses the native Web Crypto API when available (HTTPS/localhost),
// with a pure JS SHA-256 fallback for plain HTTP contexts (e.g.,
// LAN access). Both paths produce identical hashes.
...
// --- SHA-256 ---
// Uses the native Web Crypto API when available (HTTPS/localhost).
// Falls back to a pure JS implementation on plain HTTP contexts
// where crypto.subtle is undefined. Both produce identical output.
...
// Pure JS SHA-256 (FIPS 180-4) — only runs when crypto.subtle is unavailable.
function sha256Fallback(bytes: Uint8Array): string {
  ...
  for (let i = 56; i >= 0; i -= 8) padded.push((bitLen / 2 ** i) & 0xff)
