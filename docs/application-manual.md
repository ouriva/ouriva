...
- **Balance checks** — one card, two divided sections, both reconciliation figures that should read €0 when everything is settled:
  ...
  - **Non-tracked Balance** — the combined balance of all categories with `excludeFromStats: true` (configured per-category in Settings > Categories), for reconciling on-behalf-of-others spending.

  Both render through the same `BalanceIndicator` sub-component, which takes an explicit `label` prop per caller ("Total Transferred" / "Non-tracked Total") rather than a shared hardcoded string — a mislabeled shared string previously made the Non-tracked box display "Total Transferred" too.
